...
log_dir = "logs/TD3/" + SCENARIO_NAME + "/"
...
# Load model
model = TD3.load(log_dir + 'last_model')

# Initialise dict for performance tracking
keys = ["Steering (Input)", "Throttle (Input)", "Vx (m/s)", "Vy (m/s)",
...
while True:
    ...
    ego_attributes[keys[2]].append(ego.velocity.x)
    ego_attributes[keys[3]].append(ego.velocity.y)
    ego_attributes[keys[4]].append(ego.angular_velocity)
    ego_attributes[keys[5]].append(ego.angular_acceleration)
    ego_attributes[keys[6]].append(ego.heading)
    ego_attributes[keys[7]].append(ego.acceleration)
    ego_attributes[keys[8]].append(ego.jerk)
    ego_attributes[keys[9]].append(ego.dist_to_centerline)
    ego_attributes[keys[10]].append(ego.dist_to_target)
    ego_attributes[keys[11]].append(ego.dist_to_wall)
    ego_attributes[keys[12]].append(ego.tlc[0])
    ego_attributes[keys[13]].append(ego.tlc[1])
